	...
	public float getDeterminan() {
		if (getBaris() != getKolom()) {
			...
			if (getBaris() == 1) {
				...
				return getElmt(0, 0)*getElmt(1, 1) - getElmt(0, 1)*getElmt(1, 0);
			} else {
				float sum = 0;
				for (int j = 0; j < getKolom(); j++) {
					float kaliKanan = 1;
					float kaliKiri = 1;
					for (int i = 0; i < getBaris(); i++) {
						kaliKanan *= getElmt(i,  j+i < getKolom() ? j+i : j+i-getKolom());
						kaliKiri *= getElmt(i, j-i < 0 ? j-i+getKolom() : j-i);
					}
					System.out.println(kaliKanan + "	" + kaliKiri);
					sum += kaliKanan;
					sum -= kaliKiri;
				}
				return sum;
